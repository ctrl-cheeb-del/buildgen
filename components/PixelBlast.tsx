...
const PixelBlast = ({
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      // Wire up IntersectionObserver for autoPauseOffscreen
    ...
    return () => {
      ...
      t.intersectionObserver?.disconnect();
      cancelAnimationFrame(t.raf);
      t.domEl?.removeEventListener("pointerdown", t.onPointerDown);
      t.domEl?.removeEventListener("pointermove", t.onPointerMove);
      t.quad?.geometry.dispose();
      t.material.dispose();
      t.composer?.dispose();
      t.renderer.dispose();
